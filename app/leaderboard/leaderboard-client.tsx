...
export function LeaderboardClient({
    ...
    // Re-fetch via server action when user changes period or mode
    useEffect(() => {
        // Skip fetch for initial default state — we already have server data
        if (period === "global" && mode === "all") {
    ...
    return (
        <div className="flex flex-1 flex-col items-center px-4 py-8 md:px-10">
            ...
            >
                ...
                {/* Table */}
                <LeaderboardContent entries={entries} loading={isPending} />
            </motion.div>
        </div>
    );
}
